...
export default function Sidebar({ activeTab = "dashboard", setActiveTab, isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`bg-white dark:bg-[#1e293b] border-r border-slate-200 dark:border-slate-700 flex flex-col py-6 px-3 z-50 ${
          isOpen
            ? "fixed inset-y-0 left-0 z-50 w-[240px] transform translate-x-0 transition-transform duration-300 ease-in-out md:translate-x-0"
            : "hidden md:flex md:w-[240px] fixed left-0 top-0 h-full"
        }`}
      >
        <div className="mb-8 px-4 flex items-center justify-between gap-3">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="md:hidden p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 transition-colors"
            >
